...
class Grid():
    ...
    def update(self, x, y, symbol):
        ...
        return False
    # change
    def isMoveAllowed(self, x, y, symbol):
        if(self.grid[x][y] is None):
            if (symbol=="X") and ((x+y)%2==0):
                return True
            if (symbol=="O") and ((x+y)%2==1):
                return True
        return False
...
def gameLoop(screen, p1, p2):
    ...
    if(whoplayfirst=="1"):
        playerTurn = p1
        print("player1 (green) plays first")
    else:
        playerTurn = p2
    ...
    while(not gridFull(grid.grid)):
        ...
        if(playerTurn==p2):
            ...
            gui.writeScreen_4_show(screen, "[P1(Green)]:"+str(p1.get_score())+"    [P2(Red)]:"+str(p2.get_score()), line=4)
            print("Current score for player1 (green), player2 (red):",p1.get_score(),p2.get_score())
        ...
        # if the player is player1
            ...
            if p1_language =="CPP":
                ...
                grid.update(x, y, chr(playerTurn.get_symbole()))
                gui.drawSymbole(screen, (x, y), chr(playerTurn.get_symbole()))
                print("Player1 (Green,C++), move is:",x,y)
            ...
            #2.if AI written in JAVA
                ...
                grid.update(x, y, playerTurn.get_symbole())
                gui.drawSymbole(screen, (x,y), playerTurn.get_symbole())
                print("Player1 (Green,JAVA), move is:",x,y)
            ...
            print("Player1 (Green) Time:", (toc - tic))
            if (toc - tic) > TIME_LIMIT:
